...
Pipe the rule into `"${CLAUDE_CONFIG_DIR:-$HOME/.claude}/skills/always-remember/always_remember.sh"` via stdin.
...
```bash
cat <<'__ALWAYS_REMEMBER_INPUT__' | bash "${CLAUDE_CONFIG_DIR:-$HOME/.claude}/skills/always-remember/always_remember.sh" shared
<the rule, verbatim>
