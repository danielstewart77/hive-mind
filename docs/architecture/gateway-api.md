...
- **Explicit ends only**: a session is suspended or closed when somebody asks
  for it — `/kill`, `/prune`, `POST /sessions/{id}/suspend`, a rotation
  retiring its own row. Nothing ends a conversation on a timer. Inactivity is
  not evidence of abandonment, and a channel that fires weekly or a pane left
  open over a holiday are both ordinary.
- **Last-active tracking**: updated on every streamed event, and by
  `record_turn`, which is the only write a browser terminal's turn ever makes
